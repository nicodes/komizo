...
// StagedAgent is where the binary is written before agent-install.sh moves it
// into place.
//
...
//
// A minute, matching what the sampler's crontab did, because the history it
// writes is charted per minute and a slower timer would draw gaps into every
// chart. The report being up to a minute stale is what design/appify.md §3
// already accepted as the price of the account having no privileges.
